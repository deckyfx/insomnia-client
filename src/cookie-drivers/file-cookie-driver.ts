// CHANGELOG: [2025-06-04] - Created FileCookieDriver for persistent cookie storage
...
/**
 * File-based implementation of CookieDriver
 * Stores cookies persistently in a JSON file
 * Cookies persist between application sessions
 */
export class FileCookieDriver extends CookieDriver {
  private cookies: Map<string, Cookie> = new Map();
  private filePath: string;
  private isLoaded = false;

  /**
   * Creates a new FileCookieDriver instance
   * @param filePath - Path to the JSON file where cookies will be stored
   * @example
   * ```typescript
   * const driver = new FileCookieDriver('./cookies.json');
   * ```
   */
  ...
  /**
   * Loads cookies from the file if not already loaded
   */
  private async ensureLoaded(): Promise<void> {
    ...
    try {
      if (existsSync(this.filePath)) {
        ...
        const cookiesArray: Cookie[] = JSON.parse(fileContent);
      ...
      console.warn(`Failed to load cookies from ${this.filePath}:`, error);
    }
  ...
  private async saveToFile(): Promise<void> {
    try {
      ...
      const cookiesArray = Array.from(this.cookies.values());
      const fileContent = JSON.stringify(cookiesArray, null, 2);
      writeFileSync(this.filePath, fileContent, "utf-8");
    } catch (error) {
      console.warn(`Failed to save cookies to ${this.filePath}:`, error);
    }
  }
  ...
  /**
   * Removes expired cookies from storage and saves to file
   */
  ...
  async removeCookie(key: string, domain: string): Promise<boolean> {
    ...
    const cookieKey = this.getCookieKey(key, domain);
    const removed = this.cookies.delete(cookieKey);
  ...
  /**
   * Gets the file path where cookies are stored
  ...
  /**
   * Forces a reload of cookies from the file
